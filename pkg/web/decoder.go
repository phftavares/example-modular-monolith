...
func DecodeJSON(r *http.Request, destination interface{}) error {
	...
	default:
		return NewErrorf(ErrUnsupportedMediaType, "unsupported media type: %s", ct)
	}
}
...
func handleDecodeErr(err error) error {
	...
	case *json.InvalidUnmarshalError:
		return NewErrorf(ErrBadRequest, "invalid_unmarshal_error: expected=%v", e.Type)
	case *json.UnmarshalTypeError:
		return NewErrorf(ErrBadRequest,
			"unmarshal_type_error: expected=%v, got=%v, field=%v, offset=%v", e.Type, e.Value, e.Field, e.Offset)
	case *json.SyntaxError:
		return NewErrorf(ErrBadRequest, "syntax_error: offset=%v, error=%v", e.Offset, e)
	default:
		return NewErrorf(ErrBadRequest, err.Error())
	}
}
...
func handleValidateErr(err error) error {
	...
	return NewErrorf(ErrUnprocessableEntity, "validation_error: %s", message)
}
